...
function StatusPill({ status }: { status: FeatureStatus }) {
  ...
  return (
    <span className={`about-status about-status--${status}`}>{label}</span>
  );
}
...
function CreditRow({ item }: { item: CreditItem }) {
  return (
    <p className="about-credit-line">
      <span className="about-credit-label">{item.label}</span>
      {item.value}
    </p>
  );
}
...
export function AboutPage() {
  ...
  return (
    <section className="panel about-page" aria-labelledby="about-heading">
      <header className="about-section">
        ...
        <p className="about-copyright">© {year} Flexi Pomodoro</p>
      </header>

      <section className="about-section" aria-labelledby="about-notes-heading">
      ...
      <section className="about-section" aria-labelledby="about-release-heading">
        <h3 id="about-release-heading">Release</h3>
        <p className="about-release">{RELEASE_STATUS}</p>
      </section>
      ...
      <section className="about-section" aria-labelledby="about-features-heading">
        ...
        >
          <ul className="about-feature-list">
            {FEATURES.map(({ label, status }) => (
              <li key={label} className="about-feature-row">
                <span>{label}</span>
        ...
        <p className="about-footnote stub">
          Basic debug toggles are in Settings. More debug features are planned.
        </p>
      </section>

      <section className="about-section" aria-labelledby="about-links-live-heading">
        <h3 id="about-links-live-heading">Links</h3>
        <div className="about-link-card-grid">
          {LINKS_LIVE.map((card) => (
      ...
      >
        <div className="about-instance-header">
          <h3 id="about-instance-heading">Instance</h3>
          <button
            type="button"
            className="btn about-copy-btn"
        ...
        <dl className="about-instance">
          <div className="about-instance-row">
            <dt>App version</dt>
            <dd>{__APP_VERSION__}</dd>
          </div>
          <div className="about-instance-row">
            <dt>API health</dt>
            <dd className={`about-health-text about-health-text--${health.state}`}>
              {health.label}
            </dd>
          </div>
          {INSTANCE_COMING_SOON.map(({ label, value }) => (
            <div key={label} className="about-instance-row">
              <dt>{label}</dt>
              <dd className="about-instance-soon">{value}</dd>
            </div>
          ))}
        </dl>
      </section>
      ...
      <section className="about-section" aria-labelledby="about-credits-heading">
        ...
        <div className="about-credits">
          ...
          <p className="about-credit-line">
            <span className="about-credit-label">Fonts</span>
